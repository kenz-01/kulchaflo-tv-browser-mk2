...
export async function runRecommendHelperCli(argv = process.argv.slice(2), { stdout = process.stdout, stderr = process.stderr, prepare = prepareHelperEvidence, propose = proposeHelper } = {}) {
  try {
    const args = parseHelperEvidenceArguments(argv);
    if (args.help) { stdout.write(HELPER_EVIDENCE_USAGE_TEXT); return 0; }
    const bridge = prepare({ analysisPath: args.analysisPath, outputRoot: args.outputDir });
    const proposal = propose({ reportPath: bridge.outputDir, outputRoot: bridge.outputDir });
